...
class ContextualAttention(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        if self.with_fuse_correlation:
            ...
            fuse_kernel = paddle.eye(fuse_kernel_size).view(
    ...
    @staticmethod
    def calculate_unfold_hw(input_size,
                            kernel_size=3,
                            stride=1,
                            dilation=1,
                            padding=0):
    ...
    def im2col(self,
        ...
        if normalize:
            norm = paddle.sqrt((img_unfold**2).sum(dim=1, keepdim=True))
